...
def _make_proposals_generated_event(tenant_id, business_date="2026-07-28"):
    return {
...
def _make_day_closed_event(tenant_id):
    return {
...
class TestSummaryHappyPath:
    def test_creates_outbox_row_with_summary(self, seeded_db, tenant):
        ...
        event = _make_proposals_generated_event(tenant.id)
...
class TestZeroSalesDay:
    def test_summary_created_with_no_sales(self, seeded_db, tenant):
        event = _make_proposals_generated_event(tenant.id, business_date="2026-01-01")
